...
const std::vector<typename Gamesubmissions::MetaData>
    Gamesubmissions::metaData_ = {
        ...
        {"submitted_at", "::trantor::Date", "timestamp without time zone", 0, 0,
         0, 0},
        {"display_name", "std::string", "character varying", 0, 0, 0, 0}};
const std::string &Gamesubmissions::getColumnName(size_t index) noexcept(
    false) {
    assert(index < metaData_.size());
    return metaData_[index].colName_;
}
Gamesubmissions::Gamesubmissions(const Row &r,
    ...
    if (indexOffset < 0) {
        ...
        if (!r["package_type"].isNull()) {
            packageType_ = std::make_shared<std::string>(
                r["package_type"].as<std::string>());
        }
        if (!r["git_repo"].isNull()) {
            gitRepo_ =
                std::make_shared<std::string>(r["git_repo"].as<std::string>());
        }
        if (!r["review_status"].isNull()) {
            reviewStatus_ =
                std::make_shared<int32_t>(r["review_status"].as<int32_t>());
        }
...
Gamesubmissions::Gamesubmissions(
    ...
    if (!pMasqueradingVector[3].empty() &&
        ...
        if (!pJson[pMasqueradingVector[3]].isNull()) {
            gitRepo_ = std::make_shared<std::string>(
    ...
    if (!pMasqueradingVector[4].empty() &&
        ...
        if (!pJson[pMasqueradingVector[4]].isNull()) {
            reviewStatus_ = std::make_shared<int32_t>(
...
Gamesubmissions::Gamesubmissions(const Json::Value &pJson) noexcept(false) {
    ...
    if (pJson.isMember("package_type")) {
        ...
        if (!pJson["package_type"].isNull()) {
            packageType_ =
                std::make_shared<std::string>(pJson["package_type"].asString());
        }
    }
    if (pJson.isMember("git_repo")) {
        ...
        if (!pJson["git_repo"].isNull()) {
            gitRepo_ =
                std::make_shared<std::string>(pJson["git_repo"].asString());
        }
    }
    if (pJson.isMember("review_status")) {
        ...
        if (!pJson["review_status"].isNull()) {
            reviewStatus_ = std::make_shared<int32_t>(
                (int32_t)pJson["review_status"].asInt64());
        }
    }
...
void Gamesubmissions::updateByMasqueradedJson(
    const Json::Value &pJson,
    const std::vector<std::string> &pMasqueradingVector) noexcept(false) {
    if (pMasqueradingVector.size() != 7) {
        LOG_ERROR << "Bad masquerading vector";
...
const typename Gamesubmissions::PrimaryKeyType &Gamesubmissions::getPrimaryKey()
    const {
    assert(id_);
    return *id_;
}
...
const std::shared_ptr<std::string> &Gamesubmissions::getGameId()
    const noexcept {
    return gameId_;
}
...
const std::shared_ptr<std::string> &Gamesubmissions::getPackageType()
    const noexcept {
    return packageType_;
}
...
const std::shared_ptr<std::string> &Gamesubmissions::getGitRepo()
    const noexcept {
    return gitRepo_;
}
...
const std::shared_ptr<int32_t> &Gamesubmissions::getReviewStatus()
    const noexcept {
    return reviewStatus_;
}
...
const std::shared_ptr<::trantor::Date> &Gamesubmissions::getSubmittedAt()
    const noexcept {
    return submittedAt_;
}
void Gamesubmissions::setSubmittedAt(
    const ::trantor::Date &pSubmittedAt) noexcept {
...
const std::shared_ptr<std::string> &Gamesubmissions::getDisplayName()
    const noexcept {
    return displayName_;
}
...
const std::vector<std::string> &Gamesubmissions::insertColumns() noexcept {
    static const std::vector<std::string> inCols = {
        "id",          "game_id",       "package_type",
        "git_repo",    "review_status", "submitted_at",
        "display_name"};
    return inCols;
}

void Gamesubmissions::outputArgs(
...
void Gamesubmissions::updateArgs(
    drogon::orm::internal::SqlBinder &binder) const {
    if (dirtyFlag_[0]) {
        if (getId()) {
            binder << getValueOfId();
        ...
        if (!pMasqueradingVector[5].empty()) {
            if (getSubmittedAt()) {
                ret[pMasqueradingVector[5]] =
                    getSubmittedAt()->toDbStringLocal();
            } else {
...
bool Gamesubmissions::validateJsonForCreation(const Json::Value &pJson,
                                              std::string &err) {
    if (pJson.isMember("id")) {
    ...
    if (pJson.isMember("package_type")) {
        if (!validJsonOfField(2, "package_type", pJson["package_type"], err,
                              true))
            return false;
    }
    if (pJson.isMember("git_repo")) {
    ...
    if (pJson.isMember("review_status")) {
        if (!validJsonOfField(4, "review_status", pJson["review_status"], err,
                              true))
            return false;
    }
    if (pJson.isMember("submitted_at")) {
        if (!validJsonOfField(5, "submitted_at", pJson["submitted_at"], err,
                              true))
            return false;
    }
    if (pJson.isMember("display_name")) {
        if (!validJsonOfField(6, "display_name", pJson["display_name"], err,
                              true))
            return false;
    }
    return true;
}
bool Gamesubmissions::validateMasqueradedJsonForCreation(
    const Json::Value &pJson,
...
bool Gamesubmissions::validateJsonForUpdate(const Json::Value &pJson,
                                            std::string &err) {
    if (pJson.isMember("id")) {
...
bool Gamesubmissions::validateMasqueradedJsonForUpdate(
    const Json::Value &pJson,
    const std::vector<std::string> &pMasqueradingVector,
    std::string &err) {
    if (pMasqueradingVector.size() != 7) {
