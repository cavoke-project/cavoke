...
void run(const std::string &host, uint16_t port, const std::string &config_file) {
  // TODO: logging
...
int main(int argc, char* argv[]) {
  ...
  desc.add_options()
      ("help,h,?", "Print help")
      ("config-file,c", po::value<std::string>(), "File with game storage configuration")
      ("host,ip,a", po::value<std::string>()->default_value("0.0.0.0"), "Host on which server is located")
      ("port,p", po::value<uint16_t>()->default_value(8080), "TCP/IP port number for connection")
      ;
...
//    config_file = std::getenv("CAVOKE_SERVER_CONFIG"); // TODO: think and discuss
  }
